...
//   Copyright (C) 2004-2012 Per-Olof Persson. See COPYRIGHT.TXT for details.
  ...
  [x,y] = meshgrid((bbox(1, 1) : h0 : bbox(2, 1)), (bbox(1, 2) : h0 * sqrt(3) / 2 : bbox( ...
      2, 2)));
  x((2 : 2 : $), :) = x((2 : 2 : $), :) + h0 / 2; // Shift even rows
  p = [x(:),y(:)]; // List of node coordinates
  
  // 2. Remove points outside the region, apply the rejection method
  p = p( fd(p,varargin(:)) < geps, :); // Keep only d<0 points
  ...
  while 1
    ...
    Il=bars(:, [1,1,2,2]);Jl=ones(size(F)) * [1,2,1,2]; Vl=[Fvec,-Fvec];
    // Ftot = full(sparse(bars(:, [1,1,2,2]), ones(size(F)) * [1,2,1,2], [Fvec,-Fvec], N, 2));
    Ftot = full(sparse([Il(:),Jl(:)],Vl(:),[ N, 2]));
    Ftot((1 : size(pfix, 1)), :) = 0; // Force = 0 at fixed points
    p = p + deltat * Ftot; // Update node positions
    
    // 7. Bring outside points back to the boundary
    d = fd( p, varargin(:));ix = d > 0; // Find points outside (d>0)
    dgradx = ( fd( [p(ix, 1) + deps,p(ix, 2)], varargin(:)) - d(ix)) / deps;
    // Numerical
    dgrady = (fd( [p(ix, 1),p(ix, 2) + deps], varargin(:)) - d(ix)) / deps;
    //    gradient
    dgrad2 = dgradx .^ 2 + dgrady .^ 2;
    p(ix, :) = p(ix, :) - [d(ix)  .* dgradx  ./ dgrad2, ...
		    d(ix)  .* dgrady  ./ dgrad2]; // Project
